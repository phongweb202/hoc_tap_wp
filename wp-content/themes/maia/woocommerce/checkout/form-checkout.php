...
if (! $checkout->is_registration_enabled() && $checkout->is_registration_required() && ! is_user_logged_in()) {
    echo esc_html(apply_filters('woocommerce_checkout_must_be_logged_in_message', esc_html__('You must be logged in to checkout.', 'maia')));
    return;
}
...
<form name="checkout" method="post" class="<?php echo esc_attr($class_checkout); ?>" action="<?php echo esc_url(wc_get_checkout_url()); ?>" enctype="multipart/form-data">
	...
	<div class="review-wrapper col-12 col-lg-6 col-xl-6 <?php echo esc_attr($class_review); ?>">
		<div class="order-review">
			<h3 id="order_review_heading"><?php esc_html_e('Your order', 'maia'); ?></h3>

			<?php do_action('woocommerce_checkout_before_order_review'); ?>

			<div id="order_review" class="woocommerce-checkout-review-order <?php echo (maia_compatible_checkout_order()) ? 'compatible-checkout' : '';  ?>">
				<?php do_action('woocommerce_checkout_order_review'); ?>
			</div>
		</div>

		<?php if (!maia_compatible_checkout_order()) : ?>
			<div class="order-payment">
				<h3 id="order_payment_heading"><?php esc_html_e('Payment method', 'maia'); ?></h3>
