<?php
if (!defined('ABSPATH') || function_exists('Maia_Elementor_Widget_Base')) {
...
abstract class Maia_Elementor_Widget_Base extends Elementor\Widget_Base
{
    ...
    protected function get_view_template($tpl_slug, $tpl_name, $settings = [])
    {
        ...
        foreach ($templates as $template) {
            if (file_exists(MAIA_THEMEROOT . '/' . $template)) {
                $located = MAIA_THEMEROOT . '/' . $template;
                break;
            } else {
                $located = false;
            }
        ...
        if ($located) {
            ...
            echo sprintf(__('Failed to load template with slug "%s" and name "%s".', 'maia'), $tpl_slug, $tpl_name);
        }
    }
    ...
    protected function register_controls_heading($condition = array(), $full = false)
    {
        ...
        if( $full ) {
            $this->add_control(
                'heading_list_style',
                [
                    'label'         => esc_html__('Heading Style', 'maia'),
    ...
    private function register_section_styles_icon($condition) {
        $this->start_controls_section(
            ...
            [
                'label' => esc_html__( 'Heading Icon', 'maia' ),
    ...
    private function register_section_styles_content($condition)
    {
        $this->start_controls_section(
            ...
            [
                'label' => esc_html__('Heading Content', 'maia'),
                'tab'   => Controls_Manager::TAB_STYLE,
                'condition' => $condition,
            ]
        );
    ...
    private function register_section_styles_title($condition)
    {
        $this->start_controls_section(
            ...
            [
                'label' => esc_html__('Heading Title', 'maia'),
                'tab'   => Controls_Manager::TAB_STYLE,
                'condition' => $condition,
            ]
        );
    ...
    private function register_section_styles_sub_title($condition)
    {
        $this->start_controls_section(
            ...
            [
                'label' => esc_html__('Heading Sub Title', 'maia'),
                'tab'   => Controls_Manager::TAB_STYLE,
                'condition' => $condition,
            ]
        );
    ...
    private function register_section_styles_description($condition)
    {
        $this->start_controls_section(
            ...
            [
                'label' => esc_html__('Heading Description', 'maia'),
                'tab'   => Controls_Manager::TAB_STYLE,
                'condition' => $condition,
            ]
        );
    ...
    {
        return apply_filters('maia_get_template_product_grid', 'v1');
    }

    protected function get_template_product_vertical()
    {
        return apply_filters('maia_get_template_product_vertical', 'vertical-v1');
    }
    ...
    {
        $type = [
            'newest' => esc_html__('Newest Products', 'maia'),
            'on_sale' => esc_html__('On Sale Products', 'maia'),
            'best_selling' => esc_html__('Best Selling', 'maia'),
            'top_rated' => esc_html__('Top Rated', 'maia'),
            'featured' => esc_html__('Featured Product', 'maia'),
            'random_product' => esc_html__('Random Product', 'maia'),
        ];

        return apply_filters('maia_woocommerce_product_type', $type);
    }
    ...
    protected function get_product_categories($number = '')
    {
        ...
        if (!is_wp_error($category)) {
            foreach ($category as $category) {
                $results[maia_get_transliterate($category->slug)] = $category->name.' ('.$category->count.') ';
            }
        }
        return $results;
    }
    ...
    {
        $operator = [
            'AND' => esc_html__('AND', 'maia'),
            'IN' => esc_html__('IN', 'maia'),
            'NOT IN' => esc_html__('NOT IN', 'maia'),
        ];

        return apply_filters('maia_woocommerce_cat_operator', $operator);
    }

    protected function get_woo_order_by()
    {
        $oder_by = [
            'date' => esc_html__('Date', 'maia'),
            'title' => esc_html__('Title', 'maia'),
            'id' => esc_html__('ID', 'maia'),
            'popularity' => esc_html__('Popularity', 'maia'),
            'rand' => esc_html__('Random', 'maia'),
            'menu_order' => esc_html__('Menu Order', 'maia'),
        ];

        return apply_filters('maia_woocommerce_oder_by', $oder_by);
    }

    protected function get_woo_order()
    {
        $order = [
            'asc' => esc_html__('ASC', 'maia'),
            'desc' => esc_html__('DESC', 'maia'),
        ];

        return apply_filters('maia_woocommerce_order', $order);
    }
    ...
    protected function register_woocommerce_categories_operator()
    {
        ...
        $this->add_control(
            'categories',
            [
                'label' => esc_html__('Categories', 'maia'),
        ...
        $this->add_control(
            'cat_operator',
            [
                'label' => esc_html__('Category Operator', 'maia'),
                'type' => Controls_Manager::SELECT,
                'default' => 'IN',
