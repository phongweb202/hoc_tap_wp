...
class HipChatHandler extends SocketHandler
{
    ...
    public function __construct($token, $room, $name = 'Monolog', $notify = false, $level = Logger::CRITICAL, $bubble = true, $useSSL = true, $format = 'text', $host = 'api.hipchat.com', $version = self::API_V1)
    {
        if ($version == self::API_V1 && !$this->validateStringLength($name, static::MAXIMUM_NAME_LENGTH)) {
            throw new \InvalidArgumentException('The supplied name is too long. HipChat\'s v1 API supports names up to 15 UTF-8 cmaiacters.');
        }
    ...
    private function buildHeader($content)
    {
        if ($this->version == self::API_V1) {
            ...
            // needed for rooms with special (spaces, etc) cmaiacters in the name
     ...
     * allows UTF-8 cmaiacters. Otherwise, it will fall back to `strlen()`.
     *
     * Note that this might cause false failures in the specific case of using
     * a valid name with less than 16 cmaiacters, but 16 or more bytes, on a
