...
class Tbay_Widget_Popup_Newsletter extends Tbay_Widget
{
    ...
    {
        parent::__construct(
            'maia_popup_newsletter',
            esc_html__('Maia Popup Newsletter', 'maia'),
            array( 'description' => esc_html__('Show Popup Newsletter', 'maia'), )
        );
    ...
    public function scripts()
    {
        ...
        wp_enqueue_script('maia-admin', MAIA_SCRIPTS . '/admin/admin' . $suffix . '.js', array( 'jquery' ), WPTHEMBAY_ELEMENTOR_VERSION, true);
    }
    ...
    public function form($instance)
    {
        $list_socials = apply_filters(
            'maia_popup_newsletter_list_socials',array(
                'facebook'      => 'Facebook',
                'twitter'       => 'Twitter',
        ...
        $defaults = array(
            ...
            'message' => esc_html__('Message Close', 'maia'),
            'image' => '',
            'socials' => array());
        $instance = wp_parse_args((array) $instance, $defaults);
        // Widget admin form?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><strong><?php esc_html_e('Title:', 'maia'); ?></strong></label>
            <input type="text" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($instance['title']) ; ?>" class="widefat" />
        </p>
                
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('description')); ?>"><?php esc_html_e('Description:', 'maia'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('message')); ?>"><?php esc_html_e('Message Close:', 'maia'); ?></label>
        ...
        <label for="<?php echo esc_attr($this->get_field_id('image')); ?>"><?php esc_html_e('Image:', 'maia'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('socials')); ?>"><?php esc_html_e('Select socials:', 'maia'); ?></label>
            <br>
        <?php
        ...
        $link = (isset($instance['socials'][$key]['page_url'])) ? $instance['socials'][$key]['page_url']: ''; ?>
                ...
                <input class="checkbox" type="checkbox" <?php checked($checked, 1); ?> id="<?php echo esc_attr($key); ?>"
                    ...
                    <label for="<?php echo esc_attr($this->get_field_name('socials')); ?>[<?php echo esc_attr($key); ?>][status]">
                        <?php echo esc_html__('Show ', 'maia').esc_html($value); ?>
                    </label>
                ...
                <p style="display: <?php echo trim($check_value); ?>" id="<?php echo esc_attr($this->get_field_id($key)); ?>" class="text_url <?php echo esc_attr($key); ?>">
                    <label for="<?php echo esc_attr($this->get_field_name('socials')); ?>[<?php echo esc_attr($key); ?>][page_url]">
                        <?php echo esc_html($value).' '.esc_html__('Page URL:', 'maia').' '; ?>
                    </label>
